...
```json
{
  "train_args": {
    ...
    "pose_aware_position_k": 2,
    "pose_aware_direction_k": 2,
    "pose_aware_direction_radius": 3.0,
...
Pose-aware v2 selects position neighbors and direction neighbors separately. Direction neighbors must stay inside `pose_aware_direction_radius` times the median training-camera spacing, so a matching direction from an unrelated distant region is not repeated. Position and angular gaps both contribute to sampling importance. `pose_aware_extra_fraction=0.25` adds about 25% extra pool slots, while `pose_aware_max_repeat=2` prevents one camera from dominating. Set `pose_aware_test_poses` only when the pose CSV is not at the default location.
...
| `pose_aware_position_k` | Number of nearest position neighbors per test pose |
| `pose_aware_direction_k` | Number of nearest direction neighbors per test pose |
| `pose_aware_direction_radius` | Maximum normalized position gap for direction neighbors |
