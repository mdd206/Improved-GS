	...
	bool antialiasing)
{
	...
	const float c_xx_base = cov2D[0][0];
	const float c_xy_base = cov2D[0][1];
	const float c_yy_base = cov2D[1][1];
	float c_xx = c_xx_base;
	float c_xy = c_xy_base;
	float c_yy = c_yy_base;
	
	constexpr float h_var = 0.3f;
	float d_inside_root = 0.f;
	if(antialiasing)
	{
		const float det_cov = c_xx_base * c_yy_base - c_xy_base * c_xy_base;
		c_xx += h_var;
		c_yy += h_var;
	...
	if(antialiasing)
	{
		const float denom = sq(c_xx * c_yy - c_xy * c_xy);
		if (denom > 0.0f)
		{
			const float w = h_var;
			const float denom_f = d_inside_root / denom;
			dL_dc_xx = w * (w * c_yy_base + c_yy_base * c_yy_base + c_xy_base * c_xy_base) * denom_f;
			dL_dc_yy = w * (w * c_xx_base + c_xx_base * c_xx_base + c_xy_base * c_xy_base) * denom_f;
			dL_dc_xy = -2.f * w * c_xy_base * (c_xx_base + c_yy_base + w) * denom_f;
		}
	}
	...
	float* dL_dopacity)
{
	...
	// SH0 can validly contain only DC and no rest SH, but DC gradients still need to be propagated;
	// this repository's public interface always uses the dc + shs path, so using the presence of dc
	// as the color-backprop switch better matches the real behavior.
	if (dc)
...
PerGaussianRenderCUDA(
	...
	float ar[C];
	float ard;
	float prefix_color[C];
	float total_color[C];
	float prefix_invdepth;
	float total_invdepth;
	float dL_dpixel[C];
	float dL_invdepth;
	...
	for (int i = 0; i < BLOCK_SIZE + 31; ++i) {
		...
		for (int ch = 0; ch < C; ++ch) {
			...
			prefix_color[ch] = my_warp.shfl_up(prefix_color[ch], 1);
			total_color[ch] = my_warp.shfl_up(total_color[ch], 1);
			dL_dpixel[ch] = my_warp.shfl_up(dL_dpixel[ch], 1);
		}
		ard = my_warp.shfl_up(ard, 1);
		prefix_invdepth = my_warp.shfl_up(prefix_invdepth, 1);
		total_invdepth = my_warp.shfl_up(total_invdepth, 1);
		...
		if (valid_splat && valid_pixel && my_warp.thread_rank() == 0 && idx < BLOCK_SIZE) {
			...
			for (int ch = 0; ch < C; ++ch) {
				prefix_color[ch] = sampled_ar[global_bucket_idx * BLOCK_SIZE * C + ch * BLOCK_SIZE + idx];
				total_color[ch] = pixel_colors[ch * H * W + pix_id] - T_final * bg_color[ch];
			}
			ard = -pixel_invDepths[pix_id] + sampled_ard[global_bucket_idx * BLOCK_SIZE + idx];
			prefix_invdepth = sampled_ard[global_bucket_idx * BLOCK_SIZE + idx];
			total_invdepth = pixel_invDepths[pix_id];
		...
		if (valid_splat && valid_pixel && 0 <= idx && idx < BLOCK_SIZE) {
			...
			const float effective_alpha = alpha;
			float dL_deffective_alpha = 0.0f;
			const float weight = effective_alpha * T;
			...
			for (int ch = 0; ch < C; ++ch) {
				...
				dL_dalpha += ((c[ch] * T) - (1.0f / (1.0f - effective_alpha)) * (-ar[ch])) * dL_dchannel;
				bg_dot_dpixel += bg_color[ch] * dL_dpixel[ch];
			}

			ard += weight * invd;
			Register_dL_dinvdepths += weight * dL_invdepth;
			dL_dalpha += ((invd * T) - (1.0f / (1.0f - effective_alpha)) * (-ard)) * dL_invdepth;
			dL_dalpha += (-T_final / (1.0f - effective_alpha)) * bg_dot_dpixel;
			dL_deffective_alpha = dL_dalpha;
			T *= (1.0f - effective_alpha);

			// Helpful reusable temporary variables
			const float dL_dG = con_o.w * dL_deffective_alpha;
			...
			Register_dL_dopacity += G * dL_deffective_alpha;
		}
	}
