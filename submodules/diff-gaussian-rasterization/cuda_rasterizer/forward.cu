	...
	bool antialiasing)
{
	...
	// 2D covariance matrix). The radius is still used by the training
	// code, while tile duplication uses the tighter ellipse-derived bound.
	...
	float4 con_o = { conic.x, conic.y, conic.z, opacity * h_convolution_scaling };
	uint32_t tiles_count = duplicateToTilesTouched(point_image, con_o, grid, 0, 0, 0.0f, nullptr, nullptr);
	if (tiles_count == 0)
		return;

	// Store helper data only after we know the Gaussian touches at least one tile.
	depths[idx] = p_view.z;
	radii[idx] = my_radius;
	...
	float* __restrict__ accum_weights)
{
	...
	const bool record_samples = sampled_T != nullptr && sampled_ar != nullptr;
	const bool write_invdepth = depths != nullptr && invdepth != nullptr;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float test_T = T * (1 - effective_alpha);
			if (test_T < 0.0001f)
			{
				done = true;
				continue;
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += features[collected_id[j] * CHANNELS + ch] * effective_alpha * T;

			if (write_invdepth)
				expected_invdepth += (1.f / depths[collected_id[j]]) * effective_alpha * T;

			if (pixel_weights != nullptr && accum_weights != nullptr)
				atomicAdd(&accum_weights[collected_id[j]], pixel_weights[pix_id] * T * effective_alpha);
	...
	if (block.thread_rank() == 0 && max_contrib != nullptr) {
	...
	float* __restrict__ accum_max_count)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float current_weight = alpha * T;
			if (weight_max < current_weight)
			{
			...
			atomicAdd(&(accum_weights[collected_id[j]]), current_weight);
	...
	const glm::vec3* __restrict__ cam_pos)
{
	...
	float discriminant_max = 0.0f;
	float weight_max = 0.0f;
	int idx_max = 0;
	glm::vec3 point_rec = { 0.0f, 0.0f, 0.0f };

	glm::mat4 proj_matrix = glm::make_mat4x4(projmatrix);
	glm::mat4 proj_matrix_inv = glm::inverse(proj_matrix);
	float* projmatrix_inv = glm::value_ptr(proj_matrix_inv);

	glm::vec3 ray_origin = *cam_pos;
	float3 p_proj_r = { Pix2ndc(pixf.x, W), Pix2ndc(pixf.y, H), 1.0f };
	float3 p_hom_r = {
		p_proj_r.x * 1.0000001f,
		p_proj_r.y * 1.0000001f,
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			glm::vec4 q = rotations[collected_id[j]];
			float rot_r = q.x;
			float rot_x = q.y;
			float rot_y = q.z;
			float rot_z = q.w;
			...
			glm::vec3 ellipse_scales = scales[collected_id[j]] * 3.0f;
			glm::vec3 a_t = rotated_ray_direction / ellipse_scales * rotated_ray_direction / ellipse_scales;
			float a = a_t.x + a_t.y + a_t.z;
			glm::vec3 b_t = rotated_ray_direction / ellipse_scales * rotated_ray_origin / ellipse_scales;
			float b = 2.0f * (b_t.x + b_t.y + b_t.z);
			glm::vec3 c_t = rotated_ray_origin / ellipse_scales * rotated_ray_origin / ellipse_scales;
			float c = c_t.x + c_t.y + c_t.z - 1.0f;

			float discriminant = b * b - 4.0f * a * c;
			float depth = (-b / 2.0f / a) / glm::length(ray_direction);
			if (depth < 0.0f)
				continue;
			...
			if (weight_max < current_weight)
			{
				...
				idx_max = collected_id[j];
				point_rec = ray_origin + (-b / 2.0f / a) * normalized_ray_direction;
			}
...
{
	renderAuxCUDA<NUM_CHANNELS_3DGS> <<<grid, block>>> (
		ranges,
		point_list,
		W, H,
		means2D,
...
void FORWARD::render_depth(
	...
	const glm::vec3* cam_pos)
{
	renderDepthCUDA<NUM_CHANNELS_3DGS> <<<grid, block>>> (
		ranges,
		point_list,
		W, H,
		means2D,
